...
def main():
    ...
    parser.add_argument("--config", type=str, default="baseline.yaml", help="path to config which constructs model")
    parser.add_argument("--dataset", type=str, default="eeg", choices=["eeg", "meg"], help="Choose dataset: 'eeg' or 'meg'")
    ...
    parser.add_argument("--vision_backbone", type=str, help="vision_backbone")
    parser.add_argument("--c", type=int, default=6, help="c")
    parser.add_argument("--blur_kernel_size", type=int, default=None, help="blur kernel size (overrides config)")
    parser.add_argument("--clear_radius", type=float, default=None, help="clear fovea radius (overrides config)")
    parser.add_argument("--decay_rate", type=float, default=None, help="blur decay rate (overrides config)")
    opt = parser.parse_args()
    seed_everything(opt.seed)
    config = OmegaConf.load(f"{opt.config}")
    config = update_config(opt, config)
    blur_params = config['data']['blur_type']['params']
    if opt.blur_kernel_size is not None:
        blur_params['blur_kernel_size'] = opt.blur_kernel_size
    if opt.clear_radius is not None:
        blur_params['clear_radius'] = opt.clear_radius
    if opt.decay_rate is not None:
        blur_params['decay_rate'] = opt.decay_rate
    
    # ---- 更新实验名称，把模糊参数拼进去 ----
    #base_name = f"{opt.dataset}_{opt.exp_setting}_decode_{opt.brain_backbone}_{opt.vision_backbone}"
    #blur_suffix = (f"_bks{blur_params['blur_kernel_size']}"
    #              f"_cr{blur_params['clear_radius']}"
    #              f"_dr{blur_params['decay_rate']}")
    #config['name'] = base_name + blur_suffix
    ...
    if config['exp_setting'] == 'inter-subject':
        ...
        # 受试者内模式严格使用最后保存的模型（last.ckpt）进行盲测
